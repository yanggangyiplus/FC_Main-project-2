...
async def get_todos(
    skip: int = 0,
    limit: int = 1000,  # 더 많은 todos를 반환하도록 증가
    ...
    logger.info(f"[GET_TODOS] user_id={current_user.id}, email={current_user.email}")
    
    # 먼저 전체 개수 확인
    total_count = db.query(Todo).filter(
        Todo.user_id == current_user.id,
        Todo.deleted_at.is_(None)
    ).count()
    logger.info(f"[GET_TODOS] Total todos for user: {total_count}")
    ...
    logger.info(f"[GET_TODOS] Returning {len(todos)} todos (skip={skip}, limit={limit})")
    
    # 디버깅: has_notification 값 확인
    with_notification = [t for t in todos if t.has_notification]
    logger.info(f"[GET_TODOS] has_notification=True인 todos 수: {len(with_notification)}")
    if len(todos) > 0:
        sample = todos[0]
        logger.info(f"[GET_TODOS] 첫번째 todo - has_notification: {sample.has_notification} (type: {type(sample.has_notification).__name__})")
    ...
    for todo in todos:
        todo_dict = {
            ...
            # has_notification: DB 값이 truthy이거나, notification_reminders가 있으면 True
            "has_notification": True if (todo.has_notification or (todo.notification_reminders and todo.notification_reminders not in ['[]', '', None])) else False,
...
async def create_todo(
    ...
    response_data = {
        ...
        # has_notification: DB 값이 truthy이거나, notification_reminders가 있으면 True
        "has_notification": True if (db_todo_with_items.has_notification or (db_todo_with_items.notification_reminders and db_todo_with_items.notification_reminders not in ['[]', '', None])) else False,
...
async def update_todo(
    ...
    response_data = {
        ...
        # has_notification: DB 값이 truthy이거나, notification_reminders가 있으면 True
        "has_notification": True if (updated_todo.has_notification or (updated_todo.notification_reminders and updated_todo.notification_reminders not in ['[]', '', None])) else False,
