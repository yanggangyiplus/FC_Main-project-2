...
export function NotificationPanel({ 
  ...
  const notifications = useMemo(() => {
    console.log('[NotificationPanel] 전체 todos 수:', todos.length);
    
    // 첫 5개 todo의 알림 설정 확인
    todos.slice(0, 5).forEach((todo, i) => {
      console.log(`[NotificationPanel] Todo ${i}:`, {
        title: todo.title,
        hasNotification: todo.hasNotification,
        has_notification: (todo as any).has_notification,
        notificationReminders: todo.notificationReminders,
        notification_reminders: (todo as any).notification_reminders
      });
    });
    
    const filtered = todos.filter(todo => {
      const hasNotification = todo.hasNotification || (todo as any).has_notification || false;
      const notificationReminders = todo.notificationReminders || (todo as any).notification_reminders || [];
      return hasNotification && Array.isArray(notificationReminders) && notificationReminders.length > 0;
    });
    
    console.log('[NotificationPanel] 알림 설정된 todos 수:', filtered.length);
    return filtered;
  }, [todos]);
