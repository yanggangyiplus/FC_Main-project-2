...
export function DayCalendar({ todos, routines = [], familyMembers = [], selectedMembers = [], selectedDate, onDateChange, onTodoUpdate, onTodoClick }: DayCalendarProps) {
  ...
  // 드래그 중 임시 상태 (API 호출 없이 UI만 업데이트)
  const [dragPreview, setDragPreview] = useState<{ todoId: string; time: string; duration: number } | null>(null);
  ...
  const handleMouseDown = (e: React.MouseEvent, todoId: string, time: string, duration: number) => {
    e.stopPropagation();
    console.log('[DayCalendar] handleMouseDown:', { todoId, time, duration });
  ...
  // handleMouseMove는 이제 전역 이벤트(handleGlobalMouseMove)로 처리됨
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (Math.abs(deltaY) > 5) {
      ...
      // 터치 중에는 프리뷰만 업데이트
      setDragPreview({ todoId: draggedTodo, time: newTime, duration: currentDuration });
    }
  };

  // 전역 마우스 이동 핸들러 (드래그 중에만 활성화)
  const handleGlobalMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (Math.abs(deltaY) > 5) {
      ...
      console.log('[DayCalendar] handleGlobalMouseMove - setDragPreview:', { todoId: draggedTodo, time: newTime, duration: newDuration });
      setDragPreview({ todoId: draggedTodo, time: newTime, duration: newDuration });
    }
  }, [draggedTodo, dragStartY, dragStartTime, dragStartDuration, resizeMode]);

  // 전역 마우스 업 핸들러
  const handleGlobalMouseUp = useCallback(() => {
    if (draggedTodo && dragPreview && hasMoved && onTodoUpdate) {
      console.log('[DayCalendar] onTodoUpdate 호출 (mouseUp):', { todoId: dragPreview.todoId, time: dragPreview.time, duration: dragPreview.duration });
      onTodoUpdate(dragPreview.todoId, { time: dragPreview.time, duration: dragPreview.duration });
    }
    setDraggedTodo(null);
    setResizeMode(null);
    setHasMoved(false);
    setDragPreview(null);
  }, [draggedTodo, dragPreview, hasMoved, onTodoUpdate]);

  // 드래그 중에 전역 이벤트 리스너 등록
  useEffect(() => {
    if (draggedTodo) {
  ...
  }, [draggedTodo, handleGlobalMouseMove, handleGlobalMouseUp]);
  ...
  const handleTouchEnd = () => {
    // 터치를 놓을 때 API 호출
    if (draggedTodo && dragPreview && hasMoved && onTodoUpdate) {
      console.log('[DayCalendar] onTodoUpdate 호출 (touchEnd):', { todoId: dragPreview.todoId, time: dragPreview.time, duration: dragPreview.duration });
      onTodoUpdate(dragPreview.todoId, { time: dragPreview.time, duration: dragPreview.duration });
    }
    setDraggedTodo(null);
    setHasMoved(false);
        ...
        <div className="flex">
          ...
          <div className="flex-1 relative">
            ...
            <div className="absolute inset-0 px-2">
              {dayTodos.map((todo) => {
                // 드래그 중인 항목이면 프리뷰 위치/크기 사용
                const isDragging = draggedTodo === todo.id && dragPreview;
                const displayTime = isDragging ? dragPreview.time : todo.time;
                const displayDuration = isDragging ? dragPreview.duration : todo.duration;
                
                const top = getTimePosition(displayTime);
                const height = getDurationHeight(displayDuration);
                const layoutInfo = layout[todo.id] || { left: 0, width: 100 };
                
                // 디버그: 드래그 중인 항목의 상태 로깅
                if (isDragging) {
                  console.log('[DayCalendar Render] 드래그 중:', { todoId: todo.id, displayTime, displayDuration, top: `${top}%`, height: `${height}%` });
                }

                return (
                  <div
                    key={todo.id}
                    className={`absolute ${getCategoryColor(todo.category)} rounded-lg px-3 py-2 cursor-move shadow-md hover:shadow-lg ${draggedTodo === todo.id ? "opacity-80 ring-2 ring-white z-20" : "z-10 transition-all"
                  ...
                  >
                    ...
                    <div className="text-xs mt-1 opacity-90 pointer-events-none">
                      {displayTime} - {formatDuration(displayDuration || 60)}
                    </div>
