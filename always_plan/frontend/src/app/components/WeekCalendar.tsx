...
export function WeekCalendar({ todos, familyMembers = [], selectedMembers = [], selectedDate, onDateSelect, onTodoUpdate, onTodoClick }: WeekCalendarProps) {
  ...
  const handleMouseDown = (e: React.MouseEvent, todoId: string, time: string, duration: number) => {
    e.stopPropagation();
    console.log('[WeekCalendar] handleMouseDown:', { todoId, time, duration });
  ...
  // handleMouseMove는 이제 전역 이벤트로 처리됨
  ...
  const handleGlobalMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (Math.abs(deltaY) > 5) {
      ...
      if (resizeMode === 'bottom') {
        ...
        if (newDuration < 30) {
          newDuration = 30;
          newStartTotalMinutes = endTimeInMinutes - 30;
        }
      } else {
  ...
  const handleGlobalMouseUp = useCallback(() => {
    if (draggedTodo) {
      if (dragPreview && hasMoved && onTodoUpdate) {
        console.log('[WeekCalendar] onTodoUpdate 호출 (mouseUp):', { todoId: dragPreview.todoId, time: dragPreview.time, duration: dragPreview.duration });
              ...
              return (
                <div key={dayIndex} className="relative border-r border-[#F3F4F6]">
                  ...
                  {dayTodos.filter(todo => {
                    ...
                    const height = getDurationHeight(displayDuration, isAllDay);
                    ...
                    return (
                      <div
                        key={todo.id}
                        className={`absolute left-1 right-1 rounded px-1 py-1 cursor-pointer shadow-sm hover:shadow-md group ${draggedTodo === todo.id ? "opacity-70 ring-2 ring-white z-20" : "z-10 transition-shadow"
